<script lang="ts">
  ...
  function gameInput(input: string) {
    ...
    if (
      ...
      correctSequence[playerInput.length - 1]
    ) {
...
<div class="app-view space-y-6 text-center">
  ...
  <p class="text-gray-600">
    Your audio signature is confirmed. Please complete the Lather-Rinse-Repeat
    cognitive challenge to finalize your proof.
  </p>
  ...
  <div class="bg-gray-100 p-6 rounded-lg shadow-inner">
    ...
    <div class="h-6 mb-4 text-lg font-medium {gameStatusClass}">
      {gameStatus}
    </div>
  </div>
  ...
  <div class="grid grid-cols-3 gap-4">
    {#each gameButtons as button}
      <button
        on:click={() => gameInput(button)}
        disabled={buttonsDisabled}
        class="game-btn bg-blue-600 text-white font-bold text-lg py-12 rounded-lg shadow-lg hover:bg-blue-700"
      >
        {button}
      </button>
    {/each}
